...
字节码起点按头部算出的 header_len，不再硬编码 0x28——后者只在入口函数表项数
count == 1 时才等于 header_len，不为 1 的那些（buttonmenu/cgmode/config 之类 UI
脚本）会从入口表中间起步。但实测两部作品共 306 个脚本（ナギサの 144 个、終わる
世界と双子座のパラダイス 162 个）里有 60 个 count != 1，两种起点的提取结果**逐条
相同**，两侧都不抛异常：入口表里的 hash/offset 极少凑成 0x840/0x842，凑成了也过
不了 is_valid_str，而 `else: i += 1` 的逐字节步进会在遇到真正的 opcode 之前重新
对齐。所以这不是一个曾经产出乱码的 bug，改它只是为了不依赖这种巧合。对账脚本见
tests/manual/mjo_offset_diff.py。没有一个文件 count == 0，也就是说旧起点只会多
扫、不会漏掉真正的字节码。
"""
import os
import struct
